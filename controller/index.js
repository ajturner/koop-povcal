module.exports = function (povcal, createBaseController) {
  const controller = createBaseController()
  // respond to the root route
  controller.index = function (req, res) {
    res.send('Hello Povcal. Usage: FeatureServer/?country=ALB&povline=1.90')
  }
  ...
  controller.featureServer = function (req, res) {
    ...
    if (req.query.returnCountOnly === 'true') return res.send({count: 1001})
		// Fetch data from povcal's API and translate it into geojson
    povcal.get(req.query, function (err, geojson) {
